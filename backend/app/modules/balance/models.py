...
class BalanceChoice(Base):
    ...
    choice_label: Mapped[str] = mapped_column(String(10), nullable=False)   # "left" | "right"
...
class UserFearTag(Base):
    """사용자의 싫음 보관함 - 누적된 태그와 가중치"""
    __tablename__ = "user_fear_tags"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id", ondelete="CASCADE"))
    tag_id: Mapped[int] = mapped_column(Integer, ForeignKey("fear_tags.id"))
    accumulated_weight: Mapped[float] = mapped_column(Float, default=1.0)  # 선택 누적 가중치
